...
export function Toasts() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-5 right-5 z-[100] flex w-80 flex-col gap-2">
      {toasts.map((t) => (
        <div key={t.id} className={`pointer-events-auto fade-up flex items-start gap-2.5 rounded-xl border bg-navy-850/95 px-4 py-3 shadow-2xl backdrop-blur ${META[t.kind].border}`}>
          <span className="mt-0.5 shrink-0">{META[t.kind].icon}</span>
          <p className="flex-1 text-xs leading-relaxed text-mist">{t.text}</p>
          <button onClick={() => dismissToast(t.id)} className="shrink-0 text-fog hover:text-white" aria-label="Dismiss">
            <X size={13} />
          </button>
        </div>
      ))}
